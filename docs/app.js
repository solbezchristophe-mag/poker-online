...
$('#create').onclick=()=>{if(!socket?.connected)return showError('Serveur de jeu non connecté.');rememberName();$('#create').disabled=true;socket.timeout(8000).emit('createTable',{name:nameEl.value},(err,res)=>{$('#create').disabled=false;if(err)return showError('Le serveur ne répond pas.');if(!res?.ok)return showError(res?.error||'Création impossible.');enter(res.id)})};
...
function join(id){if(!socket?.connected)return showError('Serveur de jeu non connecté.');rememberName();const code=String(id||'').trim().toUpperCase();if(!code)return showError('Entre le code de la table.');socket.timeout(8000).emit('joinTable',{id:code,name:nameEl.value},(err,res)=>{if(err)return showError('Le serveur ne répond pas.');if(!res?.ok)return showError(res?.error||'Impossible de rejoindre la table.');enter(res.id)})}
function enter(id){history.replaceState(null,'',`?table=${id}`);lobby.classList.add('hidden');game.classList.remove('hidden');$('#tableId').textContent=id;showError('');playTone('click')}
